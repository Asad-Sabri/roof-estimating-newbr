...
const cardContentStyle: React.CSSProperties = { padding: "15px", fontFamily: "Arial" };
...
const PitchAndWasteTable: React.FC<{ polygons: PolygonData[], totalNetAreaSqFt: number }> = ({ polygons, totalNetAreaSqFt }) => {
    ...
    const calculatedSquares = calculatedAreas.map(area => Math.ceil(area / SQUARE_FEET_PER_SQUARE));
    ...
    return (
        <div style={{ marginBottom: '30px' }}>
            ...
            <table style={tableStyle}>
                <thead>
                    <tr>
                        ...
                        {pitchData.map((p, index) => (
                            <th key={index} style={thStyle}>Pitch {p.pitch.split('/')[0].trim()}</th>
                        ))}
                ...
                <tbody>
                    <tr>
                        <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 'bold' }}>Pitch (e.g., 3 / 12)</td>
                        {pitchData.map((p, index) => (
                            <td key={index} style={tdStyle}>{p.pitch}</td>
                        ))}
                        <td style={tdStyle}>{totalArea.toFixed(0)}</td>
                    </tr>
                    <tr>
                        <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 'bold' }}>Area (sq ft)</td>
                        {pitchData.map((p, index) => (
                            <td key={index} style={tdStyle}>{p.area.toFixed(0)}</td>
                        ))}
                        <td style={tdStyle}>{totalNetAreaSqFt.toFixed(0)}</td>
                    </tr>
                    <tr>
                        <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 'bold' }}>Percent (%)</td>
                        {pitchData.map((p, index) => (
                            <td key={index} style={tdStyle}>{p.percent.toFixed(0)}%</td>
                        ))}
                ...
                <thead>
                    <tr>
                        ...
                        {WASTAGES.map((w, index) => (
                            <th key={index} style={thStyle}>{w * 100}%</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        ...
                        {calculatedAreas.map((area, index) => (
                            <td key={index} style={tdStyle}>{area.toFixed(0)}</td>
                        ))}
                    </tr>
                    <tr>
                        <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 'bold' }}>Squares (100 sq ft)</td>
                        {calculatedSquares.map((sq, index) => (
                            <td key={index} style={tdStyle}>{sq}</td>
                        ))}
                    </tr>
                </tbody>
            </table>
...
const MeasurementsSummary: React.FC<{ summary: GAFSummary, totalArea: string, totalLength: string }> = ({ summary, totalArea, totalLength }) => {
    ...
    const summaryItems = [
        { label: "Roof Area", value: parseFloat(totalArea).toFixed(0), unit: "sq ft" },
        { label: "Roof Facets", value: summary.facetCount || 0 }, // Assuming Facet Count is available
        { label: "Pitch", value: summary.dominantPitch ? `${summary.dominantPitch} / 12` : 'N/A' }, // Assuming dominantPitch is calculated
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {summaryItems.map((item, index) => (
                <div key={index} style={itemStyle}>
                    ...
                    <span>{item.value ? (typeof item.value === 'number' ? item.value.toFixed(0) : item.value) : 0} {item.unit}</span>
                </div>
            ))}
        </div>
    );
