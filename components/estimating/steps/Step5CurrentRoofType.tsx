...
export default function Step5CurrentRoofType({
  data,
  onInputChange,
}: StepProps) {
  const category = (data.currentRoofTypeCategory || "") as "" | "flat" | "steep" | "tile" | "metal";
      ...
      <div className="mb-4">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            onInputChange("currentRoofTypeCategory", cat || undefined);
            const compatible = getEstimatingSystemsForCategory(cat);
            const currentInCompatible = compatible.some((s) => s.value === data.currentRoofType);
            if (!currentInCompatible) onInputChange("currentRoofType", undefined);
          }}
